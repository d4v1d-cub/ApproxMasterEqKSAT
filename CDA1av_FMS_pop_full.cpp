...
void init_probs(double ***&prob_joint, double ****&pu_cond, double **&pi, 
                ...
                double alpha, int K, int nch_fn, double p0, int pop_size, int ***&gamma_vals, 
                gsl_rng * r){
    double prod;
    int bit;
    ...
    for (int ch_u = 0; ch_u < nch_fn; ch_u++){
        ...
        pu_cond[ch_u] = new double **[pop_size];
        for (int elem = 0; elem < pop_size; elem++){
            ...
            pu_cond[ch_u][elem] = new double*[K];
            for (int w = 0; w < K; w++){
                pu_cond[ch_u][elem][w] = new double [2];
            }
        }
...
void fill_pos_gamma(int max_gamma, int ***gamma_vals, int nch_fn, int pop_size, int K, 
                    vector < vector <int> > *&pos_gamma){
    pos_gamma = new vector < vector <int> > [max_gamma + 1];
    for (int gamma = 0; gamma < max_gamma + 1; gamma++){
        pos_gamma[gamma] = vector < vector <int> > ();
    }
    for (int ch_u = 0; ch_u < nch_fn; ch_u++){
        for (int elem = 0; elem < pop_size; elem++){
            for (int w = 0; w < K; w++){
                pos_gamma[gamma_vals[ch_u][elem][w]].push_back(vector <int> {ch_u, elem, w});
            }
        }
    }
}
    ...
    for (int ch_u = 0; ch_u < nch_fn; ch_u++){
        for (int elem = 0; elem < pop_size; elem++){
            ...
            for (int ch = 0; ch < nch_fn; ch++){
                for (int w = 0; w < K; w++){
                    bit = ((ch >> w) & 1);
                    pi[w][bit] += prob_joint[ch_u][elem][ch];
                }
            }
            ...
            for (int w = 0; w < K; w++){
                ...
                pu_cond[ch_u][elem][w][bit] = prob_joint[ch_u][elem][ch_u] / pi[w][bit];
                pu_cond[ch_u][elem][w][1 - bit] = prob_joint[ch_u][elem][ch_uns_flip] / pi[w][1 - bit];
            }
        }
    }
}
...
void get_pu_l(double ****pu_cond, double ***pu_l, int K, int gamma, int nch_fn, int pop_size, 
              vector < vector <int> > *pos_gamma, gsl_rng * r, int &lp, int &ln){
    int index, ch_u, elem, cond_spin, bit;
    lp = 0, ln = 0;
    for (int i = 0; i < gamma; i++){
        index = gsl_rng_uniform_int(r, pos_gamma[gamma].size());
        ch_u = pos_gamma[gamma][index][0];
        elem = pos_gamma[gamma][index][1];
        cond_spin = pos_gamma[gamma][index][2];
        bit = ((ch_u >> cond_spin) & 1);
        if (bit == 0){
            pu_l[0][0][ln] = pu_cond[ch_u][elem][cond_spin][0];
            pu_l[0][1][ln] = pu_cond[ch_u][elem][cond_spin][1];
            ln++;
        }else{
            pu_l[1][0][lp] = pu_cond[ch_u][elem][cond_spin][0];
            pu_l[1][1][lp] = pu_cond[ch_u][elem][cond_spin][1];
            lp++;
        }
    }
}
...
void sum_fms(int K, int gamma, int ch_u, int plc_he, double *prob_joint, double ****pu_cond, 
             double **rates, vector < vector <int> > *pos_gamma, int nch_fn, int pop_size, double e_av, 
    ...
    int lp, ln;
    get_pu_l(pu_cond, pu_l, K, gamma, nch_fn, pop_size, pos_gamma, r, lp, ln);
...
void der_fms(double ***prob_joint, double ****pu_cond, double **rates, int pop_size, int nsampl, 
             double alpha, int K, int max_gamma, int nch_fn, double e_av, double ***me_sum, 
             int ***gamma_vals, vector < vector <int> > *pos_gamma, gsl_rng * r){
    for (int ch_u = 0; ch_u < nch_fn; ch_u++){
        for (int elem = 0; elem < pop_size; elem++){
            for (int ch = 0; ch < nch_fn; ch++){
            ...
            for (int s = 0; s < nsampl; s++){
                for (int w = 0; w < K; w++){
                    sum_fms(K, gamma_vals[ch_u][elem][w], ch_u, w, prob_joint[ch_u][elem],
                            pu_cond, rates, pos_gamma, nch_fn, pop_size, e_av, me_sum[ch_u][elem], r);
                }
            }

            for (int ch = 0; ch < nch_fn; ch++){
                me_sum[ch_u][elem][ch] /= nsampl;
            }
        }
    }
}
...
double energy(double ***prob_joint, int nch_fn, int pop_size){
    double e = 0;
    for (int ch_u = 0; ch_u < nch_fn; ch_u++){
        for (int elem = 0; elem < pop_size; elem++){
                e += prob_joint[ch_u][elem][ch_u];
        }
    }
    return e / nch_fn / pop_size;
}
...
void RK2_fms(double alpha, int K, int nch_fn, double eta, int max_gamma, int pop_size, int nsampl, 
    ...
    vector < vector <int> > *pos_gamma;
    fill_pos_gamma(max_gamma, gamma_vals, nch_fn, pop_size, K, pos_gamma);
    ...
    e = energy(prob_joint, nch_fn, pop_size) * alpha;
    ...
    while (t < tl){
        ...
        comp_pcond(prob_joint, pu_cond, pi, K, nch_fn, pop_size);

        der_fms(prob_joint, pu_cond, rates, pop_size, nsampl, alpha, K, max_gamma, nch_fn, e, me_sum, 
                gamma_vals, pos_gamma, r);   // in the rates, I use the energy density
        ...
        e = energy(prob_joint_1, nch_fn, pop_size) * alpha;
        comp_pcond(prob_joint_1, pu_cond, pi, K, nch_fn, pop_size);

        der_fms(prob_joint_1, pu_cond, rates, pop_size, nsampl, alpha, K, max_gamma, nch_fn, e, me_sum, 
                gamma_vals, pos_gamma, r);
...
int main(int argc, char *argv[]) {
    ...
    char fileener[300]; 
    sprintf(fileener, "CDA1av_pop_full_FMS_ener_K_%d_alpha_%.4lf_eta_%.4lf_tl_%.2lf_seed_%li_tol_%.1e_popsize_%d_epsc_%.e_nsampl_%d.txt", 
            K, alpha, eta, tl, seed_r, tol, pop_size, eps_c, nsampl);


    int max_gamma = get_max_gamma(alpha, K, eps_c);
    
    RK2_fms(alpha, K, nch_fn, eta, max_gamma, pop_size, nsampl, p0, fileener, tl, r, tol);
