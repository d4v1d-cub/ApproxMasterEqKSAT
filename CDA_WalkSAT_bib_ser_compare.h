...
int WhichFacNode(vector <map <int, int> > dictfacnodes, int node, int *lugvecfnode, int numvecxfacnode, Tred *red){
...
map <int, int> PlacesInFacNode(int *lugvecfnode, int node, int numvecxfacnode, vector <int> &valor_enlaces,
                               map <int, pair< int, int> > * &diccionario, Tred *red, int facnode){
    map <int, int> dict;
    dict[node] = 0;
...
void InitCadFacNode(Tred *red, int N, int K, int numvecxfacnode, int numcadenasxfactornode,
...
vector < map <int, int> > InitFacNodeInfo(Tred *red, int numvecxfacnode, int *** lugvecinoswfn, int N, int M, int K,
...
void inienlaces(Tred *red, int N, char filename[])
{
    ifstream f(filename);
    int trash;
    for (int j = 0; j < N; j++)
...
void inicred(Tred *&red,  int N, char filename[], int &maxconect, int numvecxfacnode, int *** &lugvecinoswfn) //Lee del grafo
{
    red = new Tred[N];
...
void LlenarDiccionario(Tred *red, int N, int numvecxfacnode, int *** lugvecinoswfn,
...
void InitializeToZero(Tred * red, double ***&probcond, double ** &probjoint, double ** &mejoint,
...
void InitializeToZero(Tred * red, double ** &probjoint, double ** &k1, double **&k2, int M,
...
void FillSatisfiedUnsatisfied(int N, Tred * red,  int numcadenasxfactornode,
...
void Initializecadvec(Tred *red, int N, int numvecxfacnode, int numcadenasxfactornode, int *** lugvecinoswfn,
                      map <int, pair< int, int> > * diccionario){
    // cadvec dice qué cadena ve el vecino cuando lugar ve la cadena "a"
    for (int j = 0; j < N; j++)
    {
...
void condinicrandom(double *** probcond, double ** probjoint, Tred * red, int N, int M, int K,
...
void FillRates(int N, Tred *red, int **** &tablaLocaleners, int maxconnect){
...
void FillPlaces(int N, Tred * red, int maxconnect, vector < vector <vector <int> > > &places){
